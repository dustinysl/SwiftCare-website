<%@ Page Language="C#" AutoEventWireup="true" CodeBehind="Bookings.aspx.cs" Inherits="SteVince__and_Friends_MP_FINALPROJECT.Bookings" %>
...
<head runat="server">
    ...
    <style>
        ...
        *, *::before, *::after { box-sizing: border-box; margin: 0; padding: 0; }
        ...
        .avail-section { margin-top: 16px; padding-top: 14px; border-top: 1px solid #e8f4f8; }
        .avail-section-title { font-size: 12px; font-weight: 800; color: var(--text-mid); text-transform: uppercase; letter-spacing: 0.5px; margin-bottom: 10px; }
        .avail-days-row { display: flex; flex-wrap: wrap; gap: 6px; }
        .avail-day {
        ...
        .avail-none { font-size: 13px; color: var(--text-light); font-style: italic; }
        .avail-time-row { display: flex; justify-content: space-between; align-items: center; padding: 6px 10px; background: #f4fbfc; border-radius: 8px; margin-bottom: 6px; }
        .avail-time-label { font-size: 12px; font-weight: 700; color: var(--text-mid); }
        .avail-time-val { font-size: 12px; font-weight: 800; color: var(--teal-dark); }
...
<body>
    <form id="form1" runat="server">
                        ...
                        <div class="booking-form-card">
                            ...
                            <div class="form-card-body">
                                ...
                                <div class="btn-row">
                                    ...
                                    <button type="button" class="btn-confirm" onclick="return validateAndConfirm();">Confirm Booking</button>
                        ...
                        <div class="cg-summary-card">
                            ...
                            <div class="cg-summary-body">
                                ...
                                <div class="cg-detail-row">
                                    ...
                                    <asp:Label ID="lblCgGender" runat="server" Text="N/A" />
                                </div>
                                <div class="cg-detail-row">
                                ...
                                <div class="avail-section">
                                    <div class="avail-section-title">📅 Available Days</div>
                                    <div class="avail-days-row" id="availDaysRow">
                                        <asp:Literal ID="litAvailDays" runat="server" />
                                    </div>
                                </div>

                                <!-- Availability Times -->
                                <div class="avail-section">
                                    <div class="avail-section-title">🕐 Available Hours</div>
                                    <div id="availTimesWrap">
                                        <asp:Literal ID="litAvailTimes" runat="server" />
                                    </div>
                                </div>
                            </div>
                        </div>
        ...
        <div id="confirmModal" style="display:none;position:fixed;inset:0;background:rgba(6,32,53,0.55);z-index:300;align-items:center;justify-content:center;backdrop-filter:blur(4px);">
            <div style="background:white;border-radius:20px;width:90%;max-width:460px;box-shadow:0 20px 60px rgba(10,42,74,0.25);overflow:hidden;animation:modalIn 0.3s ease;">
                <div style="background:linear-gradient(135deg,#0a2a4a,#006064);padding:24px 28px;">
                    <div style="font-family:'Montserrat',sans-serif;font-size:18px;font-weight:800;color:white;">Confirm Your Booking</div>
                    <div style="font-size:13px;color:rgba(255,255,255,0.65);margin-top:4px;">Please review your booking details below</div>
                </div>
                <div style="padding:24px 28px;">
                    <div id="modalBody" style="display:flex;flex-direction:column;gap:12px;margin-bottom:24px;"></div>
                    <div style="display:flex;gap:12px;">
                        <button type="button" onclick="closeConfirmModal()" style="flex:1;padding:12px;border-radius:10px;border:2px solid #e8f4f8;background:white;font-family:'Nunito',sans-serif;font-size:14px;font-weight:700;color:#4a7a92;cursor:pointer;">Cancel</button>
                        <button type="button" id="btnModalConfirm" style="flex:2;padding:12px;border-radius:10px;border:none;background:linear-gradient(135deg,#00838f,#1e88e5);color:white;font-family:'Nunito',sans-serif;font-size:14px;font-weight:800;cursor:pointer;">✓ Confirm Booking</button>
                    </div>
                </div>
            </div>
        </div>
        ...
        <script>
            // ── Core variables (declared first so all functions can use them) ──
            var rateRaw = parseFloat('<%= RateDecimal %>') || 0;
            var dayStart = '<%= DayStartVal %>';
            var dayEnd = '<%= DayEndVal %>';
            var availDays = <%= AvailableDaysJson %>;

            var startEl = document.getElementById('<%= txtStartTime.ClientID %>');
            var endEl = document.getElementById('<%= txtEndTime.ClientID %>');
            var dateEl  = document.getElementById('<%= txtBookingDate.ClientID %>');
            var svcEl   = document.getElementById('<%= ddlService.ClientID %>');
            ...
            // ── Time restriction ──
            function isTimeInRange(timeVal) {
                if (!dayStart || !dayEnd) return true;
                var t = toMins(timeVal);
                return t >= toMins(dayStart) && t <= toMins(dayEnd);
            }

            // ── Date restriction ──
            if (dateEl && availDays.length > 0) {
                dateEl.addEventListener('change', function () {
                    var d = new Date(this.value + 'T00:00:00');
                    if (availDays.indexOf(d.getDay()) === -1) {
                        showInlineError('⚠️ The caregiver is not available on ' + ['Sun','Mon','Tue','Wed','Thu','Fri','Sat'][d.getDay()] + '. Available days are highlighted above.');
                        this.value = '';
                    }
                });
            }
            ...
            if (startEl) {
                startEl.addEventListener('change', function () {
                    if (this.value && !isTimeInRange(this.value)) {
                        showInlineError('⚠️ Start time must be within the caregiver\'s working hours: ' + fmtTime(dayStart) + ' – ' + fmtTime(dayEnd));
                        this.value = '';
                    }
                    recalc();
                });
            ...
            if (endEl) {
                endEl.addEventListener('change', function () {
                    if (this.value && !isTimeInRange(this.value)) {
                        showInlineError('⚠️ End time must be within the caregiver\'s working hours: ' + fmtTime(dayStart) + ' – ' + fmtTime(dayEnd));
                        this.value = '';
                    }
                    recalc();
                });
            ...
            function showInlineError(msg) {
                var lbl = document.getElementById('<%= lblBookingMsg.ClientID %>');
                if (lbl) { lbl.textContent = msg; lbl.style.display = 'block'; }
                setTimeout(function () { if (lbl) lbl.style.display = 'none'; }, 4000);
            }
            ...
            function validateAndConfirm() {
                // Clear errors
                [svcEl, dateEl, startEl, endEl].forEach(function (el) { if (el) el.classList.remove('error'); });

                var ok = true;
                if (!svcEl || !svcEl.value)       { if (svcEl)   svcEl.classList.add('error');   ok = false; }
                if (!dateEl || !dateEl.value)      { if (dateEl)  dateEl.classList.add('error');  ok = false; }
                if (!startEl || !startEl.value)    { if (startEl) startEl.classList.add('error'); ok = false; }
                if (!endEl || !endEl.value)        { if (endEl)   endEl.classList.add('error');   ok = false; }

                if (!ok) { showInlineError('⚠️ Please fill in all required fields.'); return false; }

                if (toMins(endEl.value) <= toMins(startEl.value)) {
                    endEl.classList.add('error');
                    showInlineError('⚠️ End time must be after start time.');
                    return false;
                }
                ...
                if (new Date(dateEl.value) < today) {
                    ...
                    showInlineError('⚠️ Booking date cannot be in the past.');
                    return false;
                }

                // Build modal content
                var svcText   = svcEl.options[svcEl.selectedIndex].text;
                var diff      = toMins(endEl.value) - toMins(startEl.value);
                var hours     = diff / 60;
                var total     = hours * rateRaw;
                var cgName    = '<%= CaregiverDisplayName %>';
                var totalFmt  = '₱ ' + total.toLocaleString('en-PH', { minimumFractionDigits: 2, maximumFractionDigits: 2 });
                var rateFmt   = '₱ ' + rateRaw.toLocaleString('en-PH', { minimumFractionDigits: 2, maximumFractionDigits: 2 });

                var rows = [
                    { label: 'Caregiver',     value: cgName },
                    { label: 'Service',       value: svcText },
                    { label: 'Date',          value: dateEl.value },
                    { label: 'Time',          value: fmtTime(startEl.value) + ' – ' + fmtTime(endEl.value) },
                    { label: 'Hourly Rate',   value: rateFmt },
                    { label: 'Duration',      value: hours.toFixed(2) + ' hrs' },
                    { label: 'Est. Total',    value: totalFmt, highlight: true }
                ];
                ...
                rows.forEach(function (r) {
                    ...
                    row.style.cssText = 'display:flex;justify-content:space-between;align-items:center;padding:10px 14px;border-radius:10px;background:' + (r.highlight ? '#e0f7fa' : '#f4fbfc') + ';';
                    var lbl = document.createElement('span');
                    lbl.style.cssText = 'font-size:13px;font-weight:700;color:#1a4a62;';
                    lbl.textContent = r.label;
                    var val = document.createElement('span');
                    val.style.cssText = 'font-size:13px;font-weight:' + (r.highlight ? '800' : '700') + ';color:' + (r.highlight ? '#006064' : '#062035') + ';';
                ...
                // Wire confirm button to actually submit the form
                document.getElementById('btnModalConfirm').onclick = function () {
                    closeConfirmModal();
                    document.getElementById('<%= btnConfirmBooking.ClientID %>').click();
                };

                document.getElementById('confirmModal').style.display = 'flex';
                return false; // prevent immediate postback — modal handles it
            }
