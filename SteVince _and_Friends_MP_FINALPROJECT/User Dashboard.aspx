...
<head runat="server">
    ...
    <style>
        ...
        *, *::before, *::after { box-sizing: border-box; margin: 0; padding: 0; }
        ...
        .btn-review { padding:5px 14px; border-radius:8px; font-size:12px; font-weight:700; border:none; cursor:pointer; transition:all 0.2s; background:#fff8e1; color:#f57c00; white-space:nowrap; }
        .btn-review:hover { background:#f57c00; color:white; }
        .btn-reviewed { padding:5px 14px; border-radius:8px; font-size:12px; font-weight:700; border:none; background:#e8f5e9; color:#2e7d32; white-space:nowrap; cursor:default; }
        .review-modal-overlay { display:none; position:fixed; inset:0; background:rgba(6,32,53,0.55); z-index:300; align-items:center; justify-content:center; backdrop-filter:blur(4px); }
        .review-modal-overlay.open { display:flex; }
        .review-modal-box { background:white; border-radius:20px; width:90%; max-width:480px; box-shadow:0 20px 60px rgba(10,42,74,0.22); animation:modalIn 0.35s ease; overflow:hidden; }
        .review-modal-header { background:linear-gradient(135deg,var(--blue-dark),var(--teal-dark)); padding:22px 28px; position:relative; }
        .review-modal-header::before { content:""; position:absolute; inset:0; background:radial-gradient(ellipse 60% 80% at 90% 50%,rgba(38,198,218,0.18) 0%,transparent 65%); }
        .review-modal-title { font-family:"Montserrat",sans-serif; font-size:18px; font-weight:800; color:white; position:relative; z-index:1; }
        .review-modal-sub { font-size:13px; color:rgba(255,255,255,0.65); margin-top:3px; position:relative; z-index:1; }
        .review-modal-close { position:absolute; top:16px; right:20px; z-index:2; background:none; border:none; color:rgba(255,255,255,0.6); font-size:20px; cursor:pointer; transition:color 0.2s; }
        .review-modal-close:hover { color:white; }
        .review-modal-body { padding:28px; }
        .star-rating { display:flex; gap:6px; margin-bottom:8px; }
        .star-rating .star { font-size:36px; cursor:pointer; color:#d0d0d0; transition:color 0.15s,transform 0.15s; user-select:none; line-height:1; }
        .star-rating .star.active { color:#f4a535; }
        .star-rating .star:hover { transform:scale(1.15); }
        .star-label { font-size:13px; font-weight:700; color:var(--text-light); margin-bottom:16px; min-height:18px; }
        .review-textarea { width:100%; padding:12px 14px; border:2px solid #e8f4f8; border-radius:10px; font-family:"Nunito",sans-serif; font-size:14px; color:var(--text-dark); background:#f8fdff; outline:none; resize:none; height:100px; transition:all 0.25s; }
        .review-textarea:focus { border-color:var(--teal-bright); background:white; box-shadow:0 0 0 4px rgba(38,198,218,0.1); }
        .review-char-count { font-size:11px; color:var(--text-light); text-align:right; margin-top:4px; }
        .review-modal-footer { display:flex; gap:10px; margin-top:20px; }
        .btn-submit-review { flex:1; padding:13px; background:linear-gradient(135deg,var(--teal-mid),var(--blue-bright)); color:white; border:none; border-radius:50px; font-family:"Nunito",sans-serif; font-size:14px; font-weight:800; cursor:pointer; transition:all 0.3s; box-shadow:0 4px 14px rgba(38,198,218,0.3); }
        .btn-submit-review:hover { transform:translateY(-1px); }
        .btn-cancel-review { padding:13px 20px; background:white; color:var(--text-mid); border:2px solid #e8f4f8; border-radius:50px; font-family:"Nunito",sans-serif; font-size:14px; font-weight:700; cursor:pointer; transition:all 0.2s; }
        .btn-cancel-review:hover { border-color:var(--teal-bright); color:var(--teal-dark); }
        .review-server-msg { font-size:13px; color:var(--error); font-weight:700; margin-top:8px; display:block; }
    ...
    <form id="form1" runat="server">
                                ...
                                <div class="cg-list-card">
                                    <div class="cg-list-top">
                                        ...
                                        <div class="cg-list-info">
                                            ...
                                            <div class="cg-list-stars" id="stars_<%# Eval("CaregiverID") %>"></div>
                                            <script>document.getElementById('stars_<%# Eval("CaregiverID") %>').innerHTML = buildStars(<%# Eval("AvgRating") %>);</script>
                                        </div>
                            ...
                            <table class="booking-table">
                                <thead>
                                    <tr>
                                        ...
                                        <th style="padding:16px 16px 12px;">Review</th>
                                    </tr>
                                </thead>
                                <tbody>
                                    <asp:Repeater ID="rptBookings" runat="server">
                                        <ItemTemplate>
                                            <tr class="booking-row" data-status="<%# Eval("StatusClass") %>">
                                                ...
                                                <td>
                                                    <%# Eval("StatusClass").ToString() == "completed"
                                                        ? (Eval("HasReview").ToString() == "1"
                                                            ? "<span class=\"btn-reviewed\">&#10003; Reviewed</span>"
                                                            : "<button type=\"button\" class=\"btn-review\" onclick=\"openReviewModal('"
                                                              + Eval("BookingID") + "','"
                                                              + Eval("CaregiverID") + "','"
                                                              + Server.HtmlEncode(Eval("CaregiverName").ToString()) + "')\">&#11088; Leave Review</button>")
                                                        : "<span style=\"color:#c5d8e0;font-size:12px;\">&#8212;</span>"
                                                    %>
                                                </td>
                                            </tr>
                                        </ItemTemplate>
                    ...
                    <div class="modal-right">
                        ...
                        <div class="modal-section-card">
                            <div style="display:flex;align-items:center;justify-content:space-between;margin-bottom:12px;">
                                ...
                                <span style="font-size:12px;color:var(--text-light);" id="mdReviewCount"></span>
                            </div>
                            <div class="modal-rating-row">
                                <div class="modal-rating-num" id="mdRatingNum">—</div>
                                <div class="modal-rating-stars" id="mdRatingStars" style="color:#f4a535;font-size:22px;"></div>
                            </div>
                            <div id="mdReviews">
                                <div class="review-item">
                                    <div class="review-user-icon">⏳</div>
                                    <div class="review-comment">Loading reviews...</div>
                                </div>
                            </div>
                        </div>
        ...
        <script>
            ...
            function openModal(id, name, location, phone, services, rate, bio, availStatus, availDays) {
                ...
                document.getElementById('mdRatingStars').textContent = '';
                document.getElementById('mdReviewCount').textContent = '';
                document.getElementById('mdReviews').innerHTML =
                    '<div class="review-item"><div class="review-user-icon" style="font-size:13px;">⏳</div><div class="review-comment">Loading reviews...</div></div>';

                document.getElementById('cgModal').classList.add('open');
                document.getElementById('cgModal').dataset.cgId = id;

                fetch('User Dashboard.aspx/GetCaregiverReviews', {
                    method: 'POST',
                    headers: { 'Content-Type': 'application/json' },
                    body: JSON.stringify({ caregiverID: parseInt(id) })
                })
                .then(function(res) { return res.json(); })
                .then(function(data) {
                    var d = typeof data.d === 'string' ? JSON.parse(data.d) : data.d;
                    var ratingEl  = document.getElementById('mdRatingNum');
                    var starsEl   = document.getElementById('mdRatingStars');
                    var countEl   = document.getElementById('mdReviewCount');
                    var reviewsEl = document.getElementById('mdReviews');
                    ...
                    if (d.totalCount === 0) {
                        ...
                        reviewsEl.innerHTML   =
                            '<div class="review-item"><div class="review-user-icon">👤</div>' +
                            '<div class="review-comment" style="color:var(--text-light);font-style:italic;">No reviews yet. Be the first!</div></div>';
                        return;
                    }

                    ratingEl.textContent = d.avgRating.toFixed(1);
      
                    var avg = d.avgRating;
                    starsEl.innerHTML = buildStars(avg);
                    countEl.textContent = d.totalCount + (d.totalCount === 1 ? ' review' : ' reviews');
                        ...
                        item.innerHTML =
                            ...
                            '<div>' +
                                '<div style="font-weight:700;font-size:13px;color:var(--text-dark);margin-bottom:2px;">' +
                                    escHtml(r.ReviewerName) +
                                    '<span style="color:#f4a535;font-size:13px;margin-left:8px;">' + r.Stars + '</span>' +
                                '</div>' +
                                '<div class="review-comment">' +
                                    (r.Comment ? escHtml(r.Comment) : '<em style="color:var(--text-light);">No comment left.</em>') +
                                '</div>' +
                            '</div>';
                ...
                .catch(function() {
                    document.getElementById('mdReviews').innerHTML =
                        '<div class="review-item"><div class="review-user-icon">⚠️</div>' +
                        '<div class="review-comment" style="color:var(--error);">Could not load reviews.</div></div>';
                });
            }
            ...
            function buildStars(avg) {
                ...
                for (var i = 1; i <= 5; i++) {
                    if (avg >= i) {
                        ...
                        html += '<span style="color:#f4a535;font-size:20px;">★</span>';
                    } else if (avg > i - 1) {
     
                        var pct = Math.round((avg - (i - 1)) * 100);
                        html += '<span style="position:relative;display:inline-block;font-size:20px;">' +
                                    '<span style="color:#e0e0e0;">★</span>' +
                                    '<span style="position:absolute;top:0;left:0;overflow:hidden;width:' + pct + '%;color:#f4a535;">★</span>' +
                                '</span>';
                    } else {

                        html += '<span style="color:#e0e0e0;font-size:20px;">★</span>';
                    }
                }
                html += '</span>';
                return html;
            ...
            function openReviewModal(bookingID, caregiverID, caregiverName) {
                document.getElementById('<%= hfReviewBookingID.ClientID %>').value   = bookingID;
                document.getElementById('<%= hfReviewCaregiverID.ClientID %>').value = caregiverID;
                document.getElementById('reviewModalSub').textContent = 'Review for ' + caregiverName;
                currentRating = 0;
                updateStars(0);
                document.getElementById('starLabel').textContent = 'Click a star to rate';
                document.getElementById('hdnRatingValue').value = '0';
                document.getElementById('<%= txtReviewComment.ClientID %>').value = '';
                document.getElementById('reviewCharCount').textContent = '0';
                var msg = document.getElementById('<%= lblReviewMsg.ClientID %>');
            ...
            function setRating(val) {
                ...
                document.getElementById('hdnRatingValue').value = val;
            }
            ...
            document.querySelectorAll('#starRating .star').forEach(function(s) {
                ...
                s.addEventListener('mouseout',  function() { updateStars(currentRating); });
            });

            function updateReviewCount(el) {
                document.getElementById('reviewCharCount').textContent = el.value.length;
            }

            function validateReview() {
                var rating = parseInt(document.getElementById('hdnRatingValue').value || '0');
                if (rating < 1) {
                    var msg = document.getElementById('<%= lblReviewMsg.ClientID %>');
                    if (msg) msg.textContent = 'Please select a star rating.';
                    return false;
                }
                return true;
            }
        ...
        <input type="hidden" id="hdnRatingValue" name="hdnRatingValue" value="0" />
        ...
        <div class="review-modal-overlay" id="reviewModal">
            <div class="review-modal-box">
                <div class="review-modal-header">
                    ...
                    <div class="review-modal-sub" id="reviewModalSub">Leave a review for your caregiver</div>
                </div>
                <div class="review-modal-body">
                    <asp:HiddenField ID="hfReviewBookingID"   runat="server" />
                    <asp:HiddenField ID="hfReviewCaregiverID" runat="server" />
                    <label style="font-size:13px;font-weight:700;color:var(--text-mid);display:block;margin-bottom:10px;">Your Rating <span style="color:var(--error);">*</span></label>
                    ...
                    <label style="font-size:13px;font-weight:700;color:var(--text-mid);display:block;margin-bottom:6px;">Comment <span style="font-weight:400;color:var(--text-light);">(optional)</span></label>
                    <asp:TextBox ID="txtReviewComment" runat="server" CssClass="review-textarea"
                        placeholder="Share your experience with this caregiver..."
                        TextMode="MultiLine" MaxLength="500"
                        onkeyup="updateReviewCount(this)" oninput="updateReviewCount(this)" />
                    <div class="review-char-count"><span id="reviewCharCount">0</span> / 500</div>
                    <asp:Label ID="lblReviewMsg" runat="server" CssClass="review-server-msg" />
